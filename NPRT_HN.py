...
ti.sleep(3600*4)
seed = 1024
for time in range(5,12):#[2,3,4]:
    for removal_ratio in [0]:#np.arange(0, 0.46, 0.09): #np.arange(0.54, 1, 0.09):
        removal_ratio = round(removal_ratio,2)
        if time == 2 and removal_ratio == 0.0:
            print('already finished 2-0.0, continuing!')
            continue
...
#                 print("第%d个epoch的学习率：%f" % (epoch, optimizer.param_groups[0]['lr']))
        ...
        #     tmp = (loss_all / len(train_dataset)) # 算出损失值或者错误率
            ...
            if (epoch+1) % 10 == 0:
                ...
                if mae < mae_best:
                    ...
                    test = mae_test
                    best_epoch = epoch
                    np.savez(f'result/1000graph/hidden_nodes/234/acc-{removal_ratio}-{time}.npz', label=labels, preds = preds)
                    torch.save(model.state_dict(), f"result/1000graph/hidden_nodes/234/model_parameter-{removal_ratio}-{time}.pkl")
                    print(f'刷新误差率，模型已更新！')
        ...
        # save loss
        np.savez(f'result/1000graph/hidden_nodes/234/loss-{removal_ratio}-{time}.npz', loss=loss_epoch)
