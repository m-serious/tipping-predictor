...
#         g = torch.cat(gs, dim=1)
        ...
        z = self.recurrent(z) # 第一层启动运算，输入为节点及特征和边的稀疏矩阵，输出结果是二维度[20张图的所有节点数,16]
        # print(x.shape)
#         z = F.relu(z) # 激活函数
#         x = F.dropout(x, training=self.training)
#         z = self.recurrent_2(z[0]) # 第一层启动运算，输入为节点及特征和边的稀疏矩阵，输出结果是二维度[20张图的所有节点数,16]
        # print(x.shape)
#         z = F.relu(z) 
...
class GIN_GRU_cp(nn.Module):
    def __init__(self, node_features, hidden_dim, out_dim, num_layers):
        super(GIN_GRU_cp, self).__init__()
...
#         self.recurrent = GRU(hidden_dim, out_dim, 4, dropout=0.1)
        self.recurrent = GRU(int(hidden_dim/4), out_dim, 4, dropout=0.1)
...
#             nn.ReLU())
        ...
        self.project_cp = nn.Sequential(
            nn.Linear(out_dim, hidden_dim-9),
            nn.ReLU())
    ...
    def forward(self, data):
        x, edge_index, cp, batch = data.x, data.edge_index, data.cp, data.batch # 赋值
#         print(x.shape)
...
#         print(g.shape)
#         print(cp.shape)
        output = self.final( torch.cat( ( g, cp.reshape((-1,9)) ),1 ) )
#         g = self.before_final( torch.cat( ( g, cp ),1 ) )
#         output = self.bn_last(g)
#         print(torch.sum(output))
