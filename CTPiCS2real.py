...
#     val_dataset = val_test_dataset[:len(val_test_dataset)-int(len(val_test_dataset)*2/3)]
    ...
    train_loader = DataLoader(train_dataset, batch_size=16,shuffle=True) # 加载训练数据集，训练数据中分成每批次20个图片data数据
    ...
    loss_epoch = np.zeros(500)
    for epoch in range(500): # 训练所有训练数据集20次
        loss_all = 0
        # 一轮epoch优化的内容
...
#                 print("第%d个epoch的学习率：%f" % (epoch, optimizer.param_groups[0]['lr']))
    ...
    #     tmp = (loss_all / len(train_dataset)) # 算出损失值或者错误率
        ...
        if (epoch+1) % 10 == 0:
            ...
            val_loader = DataLoader(test_dataset, batch_size=len(test_dataset), shuffle=False) # 加载训练数据集，训练数据中分成每批次20个图片data数据
            labels, preds = [], []
...
#                 test = mae_test
                best_epoch = epoch
                np.savez(f'prediction/CTPiCS/2real/acc-{time}-16.npz', label=labels, preds = preds)  # hidden_nodes
                torch.save(model.state_dict(), f"prediction/CTPiCS/2real/model_parameter-{time}-16.pkl")
                print(f'刷新误差率，模型已更新！')
            print(f'times={time}, epoch:{epoch+1}, 当前所有测试，在验证集上的绝对值误差最好为:{mae_best}, best epoch:{best_epoch+1}')
    ...
    # save loss
    np.savez(f'prediction/CTPiCS/2real/loss-{time}-16.npz', loss=loss_epoch)
